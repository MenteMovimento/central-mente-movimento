...
const CENTRAL_LANGUAGE_STORAGE_KEY = "central-language";
const DISPOSITIVOS_LANGUAGE_STORAGE_KEY = "mentemovimento-language";
...
const translations = {
  ...
  en: {
    ...
    app: {
      title: "Member Management | MenteMovimento",
...
function loadStoredLanguage() {
  try {
    const stored =
      localStorage.getItem(CENTRAL_LANGUAGE_STORAGE_KEY) ||
      localStorage.getItem(LANGUAGE_STORAGE_KEY) ||
      localStorage.getItem(DISPOSITIVOS_LANGUAGE_STORAGE_KEY);
...
function persistStoredLanguage(language) {
  try {
    [CENTRAL_LANGUAGE_STORAGE_KEY, LANGUAGE_STORAGE_KEY, DISPOSITIVOS_LANGUAGE_STORAGE_KEY].forEach((key) => {
...
function applyLanguage(language = loadStoredLanguage(), persist = true) {
  ...
  if (persist) {
    persistStoredLanguage(resolvedLanguage);
  }
...
async function initApp() {
  ...
  window.addEventListener("storage", (event) => {
    if ([CENTRAL_LANGUAGE_STORAGE_KEY, LANGUAGE_STORAGE_KEY, DISPOSITIVOS_LANGUAGE_STORAGE_KEY].includes(event.key)) {
      const nextLanguage = loadStoredLanguage();
